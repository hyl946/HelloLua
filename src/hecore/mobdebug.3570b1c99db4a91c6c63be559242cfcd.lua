...
function debug_hook(event, line)
  ...
  elseif event == "line" then
    ...
    --if _G.isLocalDevelopMode then printx(0, (lastfile or "nil") .. " " .. (lastsource or "nil") .. " " .. caller.source .. " " .. event .. " " .. line) end
    ...
    if status and res and res ~= 'stack' then
      if abort == nil and res == "exit" then
        if _G.isLocalDevelopMode then printx(0, "******* exit *******") end
        lua_stub_exit(0)
        if _G.isLocalDevelopMode then printx(0, "******* exit *******") end
        return
      end
...
local function debugger_loop(sfile, sline)
  ...
  while true do
    ...
    while true do
      ...
      --if _G.isLocalDevelopMode then printx(0, line) end
    ...
    elseif command == "STEP" then
      ...
      local ev, vars, file, line, idx_watch = coroutine.yield()
      bp_wapper:step()
      --if _G.isLocalDevelopMode then printx(0, "STEP file:"..file) end
...
local function start(controller_host, controller_port)
  ...
  if server then
    ...
    file = string.gsub( file, '^'..q(basedir),'')
    if _G.isLocalDevelopMode then printx(0, "start debug file:"..file) end
    ...
    if _G.isLocalDevelopMode then printx(0, "Could not connect to " .. controller_host .. ":" .. controller_port) end
  end
end
...
local function handle(params, client, options)
  ...
  or command == "over" or command == "exit" then
    ...
    while true do
      ...
      if not breakpoint then
        if _G.isLocalDevelopMode then printx(0, "Program finished") end
        os.exit()
      ...
      elseif status == "202" then
        ...
        if file and line then
          if _G.isLocalDevelopMode then printx(0, "Paused at file " .. file .. " line " .. line) end
        end
      elseif status == "203" then
        _, _, file, line, watch_idx = string.find(breakpoint, "^203 Paused%s+(.-)%s+(%d+)%s+(%d+)%s*$")
        if file and line and watch_idx then
          if _G.isLocalDevelopMode then printx(0, "Paused at file " .. file .. " line " .. line .. " (watch expression " .. watch_idx .. ": [" .. watches[watch_idx] .. "])") end
        end
      elseif status == "204" then
        ...
        if stream and size then
          ...
          if _G.isLocalDevelopMode then printx(0, msg) end
      ...
      elseif status == "401" then
        ...
        if size then
          local msg = client:receive(tonumber(size))
          if _G.isLocalDevelopMode then printx(0, "Error in remote application: " .. msg) end
          os.exit(1)
          return nil, nil, msg -- use return here for those cases where os.exit() is not wanted
        end
      else
        if _G.isLocalDevelopMode then printx(0, "Unknown error") end
        os.exit(1)
  ...
  elseif command == "setb" then
    ...
    if file and line then
      ...
      if client:receive() == "200 OK" then 
        ...
        if _G.isLocalDevelopMode then printx(0, "Error: breakpoint not inserted") end
      end
    else
      if _G.isLocalDevelopMode then printx(0, "Invalid command") end
    end
  elseif command == "setw" then
    ...
    if exp then
      ...
      if watch_idx then
        ...
        if _G.isLocalDevelopMode then printx(0, "Inserted watch exp no. " .. watch_idx) end
      else
        local _, _, size = string.find(answer, "^401 Error in Expression (%d+)$")
        if size then
          local err = client:receive(tonumber(size)):gsub(".-:%d+:%s*","")
          if _G.isLocalDevelopMode then printx(0, "Error: watch expression not set: " .. err) end
        else
          if _G.isLocalDevelopMode then printx(0, "Error: watch expression not set") end
        end
      end
    else
      if _G.isLocalDevelopMode then printx(0, "Invalid command") end
  ...
  elseif command == "delb" then
    ...
    if file and line then
      ...
      if client:receive() == "200 OK" then 
        ...
        if _G.isLocalDevelopMode then printx(0, "Error: breakpoint not removed") end
      end
    else
      if _G.isLocalDevelopMode then printx(0, "Invalid command") end
    end
    ...
    for file, breaks in pairs(breakpoints) do
      for line, _ in pairs(breaks) do
        ...
        if client:receive() == "200 OK" then 
          ...
          if _G.isLocalDevelopMode then printx(0, "Error: breakpoint at file " .. file .. " line " .. line .. " not removed") end
        end
      end
    end
  elseif command == "delw" then
    ...
    if index then
      ...
      if client:receive() == "200 OK" then 
        ...
        if _G.isLocalDevelopMode then printx(0, "Error: watch expression not removed") end
      end
    else
      if _G.isLocalDevelopMode then printx(0, "Invalid command") end
    end
  elseif command == "delallw" then
    for index, exp in pairs(watches) do
      ...
      if client:receive() == "200 OK" then 
        ...
        if _G.isLocalDevelopMode then printx(0, "Error: watch expression at index " .. index .. " [" .. exp .. "] not removed") end
      end
    end    
      ...
      while true do
        ...
        if status == "200" then
          ...
          if len > 0 then
            ...
            if err then
              if _G.isLocalDevelopMode then printx(0, "Error in processing results: " .. err) end
              return nil, nil, "Error in processing results: " .. err
            end
            if _G.isLocalDevelopMode then printx(0, (table.unpack or unpack)(res)) end
            return res[1], res
          end
        ...
        elseif status == "401" then
          ...
          if _G.isLocalDevelopMode then printx(0, "Error in expression: " .. res) end
          return nil, nil, res
        else
          if _G.isLocalDevelopMode then printx(0, "Unknown error") end
  ...
  elseif command == "listb" then
    for k, v in pairs(breakpoints) do
      ...
      if _G.isLocalDevelopMode then printx(0, b) end
    end
  elseif command == "listw" then
    for i, v in pairs(watches) do
      if _G.isLocalDevelopMode then printx(0, "Watch exp. " .. i .. ": " .. v) end
    end    
  elseif command == "suspend" then
    client:send("SUSPEND\n")
  elseif command == "stack" then
    ...
    if status == "200" then
      ...
      if func == nil then
        if _G.isLocalDevelopMode then printx(0, "Error in stack information: " .. err) end
        return nil, nil, err
      end
      local ok, stack = pcall(func)
      if not ok then
        if _G.isLocalDevelopMode then printx(0, "Error in stack information: " .. stack) end
      ...
      for _,frame in ipairs(stack) do
        ...
        if _G.isLocalDevelopMode then printx(0, serpent.line(frame[1], {comment = false})) end
      end
      return stack
    elseif status == "401" then
      local res = "Error in stack result"
      if _G.isLocalDevelopMode then printx(0, res) end
      return nil, nil, res
    else
      if _G.isLocalDevelopMode then printx(0, "Unknown error") end
  ...
  elseif command == "output" then
    ...
    if stream and mode then
      ...
      if status == "200" then
        if _G.isLocalDevelopMode then printx(0, "Stream "..stream.." redirected") end
  ...
  elseif command == "basedir" then
    ...
    if dir then
      ...
      local _, _, status = string.find(resp, "^(%d+)%s+%w+%s*$")
      if status == "200" then
        if _G.isLocalDevelopMode then printx(0, "New base directory is " .. basedir) end
      else
        if _G.isLocalDevelopMode then printx(0, "Unknown error") end
        return nil, nil, "Debugger error: unexpected response after BASEDIR"
      end
    else
      if _G.isLocalDevelopMode then printx(0, basedir) end
    end
  elseif command == "help" then
    if _G.isLocalDevelopMode then printx(0, "setb <file> <line>    -- sets a breakpoint") end
    if _G.isLocalDevelopMode then printx(0, "delb <file> <line>    -- removes a breakpoint") end
    if _G.isLocalDevelopMode then printx(0, "delallb               -- removes all breakpoints") end
    if _G.isLocalDevelopMode then printx(0, "setw <exp>            -- adds a new watch expression") end
    if _G.isLocalDevelopMode then printx(0, "delw <index>          -- removes the watch expression at index") end
    if _G.isLocalDevelopMode then printx(0, "delallw               -- removes all watch expressions") end
    if _G.isLocalDevelopMode then printx(0, "run                   -- runs until next breakpoint") end
    if _G.isLocalDevelopMode then printx(0, "step                  -- runs until next line, stepping into function calls") end
    if _G.isLocalDevelopMode then printx(0, "over                  -- runs until next line, stepping over function calls") end
    if _G.isLocalDevelopMode then printx(0, "out                   -- runs until line after returning from current function") end
    if _G.isLocalDevelopMode then printx(0, "listb                 -- lists breakpoints") end
    if _G.isLocalDevelopMode then printx(0, "listw                 -- lists watch expressions") end
    if _G.isLocalDevelopMode then printx(0, "eval <exp>            -- evaluates expression on the current context and returns its value") end
    if _G.isLocalDevelopMode then printx(0, "exec <stmt>           -- executes statement on the current context") end
    if _G.isLocalDevelopMode then printx(0, "load <file>           -- loads a local file for debugging") end
    if _G.isLocalDevelopMode then printx(0, "reload                -- restarts the current debugging session") end
    if _G.isLocalDevelopMode then printx(0, "stack                 -- reports stack trace") end
    if _G.isLocalDevelopMode then printx(0, "output stdout <d|c|r> -- capture and redirect io stream (default|copy|redirect)") end
    if _G.isLocalDevelopMode then printx(0, "basedir [<path>]      -- sets the base path of the remote application, or shows the current one") end
    if _G.isLocalDevelopMode then printx(0, "exit                  -- exits debugger") end
  else
...
local function listen(host, port)
  ...
  if _G.isLocalDevelopMode then printx(0, "Lua Remote Debugger") end
  if _G.isLocalDevelopMode then printx(0, "Run the program you wish to debug") end
  ...
  if file and line then
    if _G.isLocalDevelopMode then printx(0, "Paused at file " .. file ) end
    if _G.isLocalDevelopMode then printx(0, "Type 'help' for commands") end
  else
    local _, _, size = string.find(breakpoint, "^401 Error in Execution (%d+)%s*$")
    if size then
      if _G.isLocalDevelopMode then printx(0, "Error in remote application: ") end
      if _G.isLocalDevelopMode then printx(0, client:receive(size)) end
    end
  end
