---------------------------------------------------------------------------------------
-- @Author: dan.liang
-- @Date:   2018-09-04 16:12:16
-- @Email:  dan.liang@happyelements.com
-- @Last Modified by:   dan.liang
-- @Last Modified time: 2018-09-18 16:46:43
---------------------------------------------------------------------------------------
-- 活动面板
...
ActivityCenterPanel = class(BasePanel)
...
function ActivityCenterPanel:create(defaultData)
	local s = ActivityCenterPanel.new()
	s.defaultData = defaultData
	s:init()
	return s	
end

function ActivityCenterPanel:ctor()

end

function ActivityCenterPanel:dispose( ... )
	BasePanel.dispose(self)
	ActivityCenter:unloadResources()
end


function ActivityCenterPanel:init(...)
	-- local visibleOrigin = Director:sharedDirector():getVisibleOrigin()
	-- local visibleSize = Director:sharedDirector():getVisibleSize()
	
	self.builder = ActivityCenter:loadResources("ui/ActivityCenter/ActivityCenterPanel.json")

    local ui = self.builder:buildGroup("ActivityCenterPanel_2")
    ...
    local bottomHeight = 0 --20
    local topHeight = 0 --240
    ...
    local contentContainer = topUi:getChildByName("container")
    local size = contentContainer:getGroupBounds(topUi).size
    contentContainer:setVisible(false)

    local vbw = size.width
    local vbh = size.height
    local vbp = contentContainer:getPosition()
    ...
    local itemsSize = itemsPh:getGroupBounds(topUi).size
    local pos = itemsPh:getPosition()

    local clipping = SimpleClippingNode:create()
    clipping:setContentSize(CCSizeMake(itemsSize.width, itemsSize.height))
    ...
    local itemsLayer = LayerColor:createWithColor(ccc3(255, 0, 0), 1000, 200)--Layer:create()
    itemsLayer:setOpacity(0)
    itemsLayer:setPositionY(-20)
    ...
    self.itemW = 142
    self.itemWA = self.itemW--(itemsSize.width - 4 * self.itemW) / 3 + self.itemW + 2
    self.items = {}
    self.touchCount = 0
...
function ActivityCenterPanel:isNetworkEnabled( ... )
	if __IOS then
...
function ActivityCenterPanel:leftArrowClick(count)
...
function ActivityCenterPanel:rightArrowClick(count)
...
function ActivityCenterPanel:updateArrow()
...
function ActivityCenterPanel:buildItem(itemData)
	local item = self.builder:buildGroup("ActivityCenter/panelStyle/item")
	item.data = itemData
	item.id = itemData.id
	...
	local function CreateIcon( isUp )
		...
		simg:setPositionXY(sph_pos.x + sph_size.width/2, sph_pos.y-3)
		item:addChild(simg)
		...
		stxt:setPositionXY(sph_pos.x + sph_size.width/2, sph_pos.y - sph_size.height - 1)
		item:addChild(stxt)
		return simg,stxt
	end
...
function ActivityCenterPanel:getItemById( id )
...
function ActivityCenterPanel:checkContainItem( itemOrData )
...
function ActivityCenterPanel:insertItem( item, index)
...
function ActivityCenterPanel:onTouchItem( indexOrData )
	--[[
...
function ActivityCenterPanel:showItem( item )
...
--更新标签面板
function ActivityCenterPanel:refreshPanel(data)
...
--更新标签页icon
function ActivityCenterPanel:refreshItem()
...
function ActivityCenterPanel:refreshItemStatus( data )
	local item = self:getItemById(data.id)
	if item then
		local hasAward = ActivityCenter:hasRewardMark(item.data)
		local msgNum = ActivityCenter:getMsgNum(item.data)
...
function ActivityCenterPanel:onDownloadResProcess( data, info )
	local item = self:getItemById(data.id)
	if item and item == self.curItem then
...
function ActivityCenterPanel:getPriority( data )
...
function ActivityCenterPanel:onKeyBackClicked()
	ActivityCenter:exitActivityCenter()
end

function ActivityCenterPanel:popout()
...
function ActivityCenterPanel:removePopout()
