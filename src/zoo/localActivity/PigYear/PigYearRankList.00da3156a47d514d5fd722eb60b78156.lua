...
PigYearRankListTableViewRender = class(TableViewRenderer)

function PigYearRankListTableViewRender:init(rankListDataCache, cacheRankType, levelFlag, ...)
...
function PigYearRankListTableViewRender:dispose(  )
...
function PigYearRankListTableViewRender:create(rankListDataCache, cacheRankType, width, height, levelFlag ,...)
	assert(rankListDataCache)
	assert(cacheRankType)
	RankListCacheRankType.checkRankType(cacheRankType)
	...
	local render = PigYearRankListTableViewRender.new(width, height)
	render:init(rankListDataCache, cacheRankType , levelFlag )
	return render
end

function PigYearRankListTableViewRender:buildCell(cell, index, ...)
	assert(cell)
	...
	-----------
	-- rankListItem.userScoreLabel:setVisible(false)
	local tx,ty = 380,rankListItem.userScoreLabel:getPositionY()-15

	local color = ccc3(178,88,47)

	local node = CocosObject:create()
    rankListItem.ui:addChild(node)
    rankListItem.rankNode = node

    local text = TextField:create("余", nil, 24, CCSizeMake( 100 , 0), nil, kCCVerticalTextAlignmentRight)
    text:setColor(color)
    text:setPositionXY(tx,ty)
    node:addChild(text)

    local text = TextField:create("步", nil, 24, CCSizeMake( 100 , 0), nil, kCCVerticalTextAlignmentLeft)
    text:setColor(color)
    text:setPositionXY(tx+110,ty)
    node:addChild(text)

    local text = BitmapText:create("0", "fnt/newzhousai_rubynum.fnt")
    text:setAnchorPoint(ccp(0.5,0.5))
    text:setScale(0.6)
--    text.ori_pos_x = tx-8
--    text.ori_pos_y = ty+23
     text:setPositionXY(tx+20,ty)
--    text.width = 100
--    text.height = 45
    node:addChild(text)
    rankListItem.txtStep = text


    local text = TextField:create("未上榜", nil, 20, CCSizeMake( 100 , 0), nil, kCCVerticalTextAlignmentCenter)
    text:setColor(color)
    text:setPositionXY(70,ty)
    rankListItem.ui:addChild(text)
    rankListItem.txtNoRank = text
	...
	for i,v in ipairs(list) do
		rankListItem[v]:removeFromParentAndCleanup(false)
		node:addChild(rankListItem[v])
	end


    local size = rankListItem.userNameLabel:getDimensions()
    rankListItem.userNameLabel:setDimensions(CCSizeMake(size.width-50,size.height))
    -----------
...
function PigYearRankListTableViewRender:getContentSize(tableView, index, ...)
	assert(tableView)
	assert(type(index) == "number")
	assert(#{...} == 0)
...
function PigYearRankListTableViewRender:setData(rawCocosObj, index, ...)
	assert(rawCocosObj)
	assert(index)
	assert(#{...} == 0)
	...
	rankListItemToControl:setData(index, userName, 0, data.headUrl, uid == data.uid, data.star, data.uid, data)

	-- rankListItemToControl.userScoreLabel:setString("还剩" .. tostring(data.score) .. "步")
	local isRank = data.score and data.score>=0
	rankListItemToControl.rankNode:setVisible(isRank)
	rankListItemToControl.txtNoRank:setVisible(not isRank)
	rankListItemToControl.userScoreLabel:setVisible(not isRank)
	
	if data.score<0 then
		rankListItemToControl.userScoreLabel:setString("闯关中")
	else
		 rankListItemToControl.txtStep:setText(data.score)
--	    UIHelper:setLeftText(rankListItemToControl.txtStep, tostring(data.score))
	end
end

function PigYearRankListTableViewRender:numberOfCells(...)
	assert(#{...} == 0)
...
function PigYearRankList:init(levelId, panelWithRank, ...)
	assert(levelId)
	assert(#{...} == 0)
	...
	self.friendRankBtnRes:setVisible(false)
	self.serverRankBtnRes:setVisible(false)
	...
	local function onRankListCachedDataChange(rankType, ...)
		...
		-- elseif rankType == RankListCacheRankType.FRIEND then
			...
			if self.friendRankListTableView.isDisposed or  not self.friendRankListTableView.refCocosObj then
				return 
			end
	...
	-- self.serverPigYearRankListTableViewRender	= PigYearRankListTableViewRender:create(self.rankListCache, RankListCacheRankType.SERVER,
	-- 	rankListItemWidth, rankListItemHeight, self.levelFlag )
	-- self.serverPigYearRankListTableViewRender.owner = self
	-- self.serverRankListTableView		= TableView:create(self.serverPigYearRankListTableViewRender, rankListItemWidth, rankListItemHeight * 6.8)
	-- self.serverRankListTableView		= TableView:create(self.serverPigYearRankListTableViewRender, rankListItemWidth, size.height - 40)
	--self.serverRankListTableView:setTouchEnabled(false)

	--------------------
	-- Craete Friend Rank List
	-- -----------------
	self.friendPigYearRankListTableViewRender	= PigYearRankListTableViewRender:create(self.rankListCache, RankListCacheRankType.FRIEND,
		rankListItemWidth, rankListItemHeight, self.levelFlag )
	self.friendPigYearRankListTableViewRender.owner = self
	-- self.friendRankListTableView		= TableView:create(self.friendPigYearRankListTableViewRender, rankListItemWidth, rankListItemHeight * 6.8)
	self.friendRankListTableView		= TableView:create(self.friendPigYearRankListTableViewRender, rankListItemWidth, size.height - 110)
...
function PigYearRankList:initFriendRankRefreshBtn(ui)
...
-- function PigYearRankList:showStrategyGuide()
...
function PigYearRankList:setButtonsEnable(enable)
end

-- Called By PanelWithRankExchangeAnim / PanelWithRankPopRemoveAnim, When This Rank List Is 
-- About To Poping Out
function PigYearRankList:prePopoutCallback(...)
	assert(#{...} == 0)
...
function PigYearRankList:postPopoutCallback()
...
function PigYearRankList:getPanelWithRankId()
	local panelId 
...
function PigYearRankList:updateNoNetWorkLabel()
	if self.isDisposed then
		return
	end
...
function PigYearRankList:ShowNoNetWorkLabel()
	if self.isDisposed then
		return
	end
...
function PigYearRankList:ShowActIsOverLabel()
	if self.isDisposed then
		return
	end

	self.noNetworkLabel:setString("活动已结束")
	self.noNetworkLabel:setVisible(true)
end


function PigYearRankList:updateWhenDataChange(...)
	print("-------PigYearRankList:updateWhenDataChange",self.friendRankListTableView:isVisible())
	assert(#{...} == 0)
...
function PigYearRankList:createLineStar(width, height)
...
function PigYearRankList:playChangeRankAnim(oldNumber, newNumber, animFinishCallback, playScaleAnim, ...)
...
function PigYearRankList:onServerBtnTapped()
	--if self.isServerRankHasData then
...
function PigYearRankList:onFriendBtnTapped()
	-- self.serverRankListTableView:setVisible(false)
...
function PigYearRankList:onStrategyBtnTapped()
...
function PigYearRankList:changeLabelToServerRank(playScaleAnim, ...)
	assert(#{...} == 0)
	-- Cur Server Rank
	...
	if _G.isLocalDevelopMode then printx(0, "PigYearRankList:changeLabelToServerRank Called !") end

	self:playChangeRankAnim(oldServerRank, curServerRank, false, playScaleAnim)
end

function PigYearRankList:changeLabelToFriendRank(playScaleAnim, ...)
	assert(#{...} == 0)
	...
	-- if _G.isLocalDevelopMode then printx(0, "PigYearRankList:changeLabelToFriendRank Called !") end
...
function PigYearRankList:setTableViewTouchEnable(bool, ...)
...
function PigYearRankList:setTableViewBounceable(bounceable, ...)
...
function PigYearRankList:getVisibleRankListTableView(...)
	assert(#{...} == 0)
...
function PigYearRankList:getScale9Bg(...)
	assert(#{...} == 0)

	return self.scale9Bg
end

function PigYearRankList:getRankListItemBg(...)
	assert(#{...} == 0)

	return self.rankListItemBg
end

function PigYearRankList:create(levelId, panelWithRank, ...)
	local newRankList = PigYearRankList.new()
	newRankList:init(levelId, panelWithRank)
	return newRankList
end


function PigYearRankList:dispose()
	CCSpriteFrameCache:sharedSpriteFrameCache():removeSpriteFramesFromFile(getRealPlistPath("flash/four_star_shine.plist"))
	-- self.serverPigYearRankListTableViewRender:dispose()
	self.friendPigYearRankListTableViewRender:dispose()
	BasePanel.dispose(self)
end

-- function PigYearRankList:setMainBGColorWithData_Purple()
-- 	--色度 
-- 	--饱和度
-- 	--亮度
-- 	--对比度
...
-- 	local mainBG = self.ui:getChildByName("scale9Bg"):getChildByName("图层 13")
-- 	if mainBG then
...
-- end
-- function PigYearRankList:setMainBGColorWithData_Green()
