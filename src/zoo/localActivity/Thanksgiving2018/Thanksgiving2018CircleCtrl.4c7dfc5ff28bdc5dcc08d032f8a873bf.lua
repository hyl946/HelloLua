...
local Thanksgiving2018CircleCtrl = class()

--kTextAlignment = {kCCTextAlignmentLeft, kCCTextAlignmentCenter, kCCTextAlignmentRight}

function Thanksgiving2018CircleCtrl:create( parent, pos, initShowNum, FntFile, Scale, TextAlignment, maxShowLength, fntWidth )
	local bar = Thanksgiving2018CircleCtrl.new()
	bar:init( parent, pos, initShowNum, FntFile, Scale,TextAlignment, maxShowLength, fntWidth  )
	return bar
end

--maxShowLength 最大显示字的位数 3位最大只显示 999
function Thanksgiving2018CircleCtrl:init( parent, pos, initShowNum, FntFile, Scale,TextAlignment, maxShowLength, fntWidth  )
...
function Thanksgiving2018CircleCtrl:getNumList( num )
    --拆分数字
    local NumberList = {}
...
function Thanksgiving2018CircleCtrl:getMaxShowNum( maxShowLength )
    --计算最大可显示数
    local maxnum = 1
    for i=1, maxShowLength do
        maxnum = maxnum * 10
...
function Thanksgiving2018CircleCtrl:UpdateMainFntPos()
...
function Thanksgiving2018CircleCtrl:setNumber( num, bAnim, bAniEndCallBack )
...
--现在只支持123456这么走
function Thanksgiving2018CircleCtrl:UpdateNumber()
...
function Thanksgiving2018CircleCtrl:UpdateFntPos( LabelList, ShowNum )
...
return Thanksgiving2018CircleCtrl
