...
function SharePicture:capture( ui )
	local shareImagePath = self:captureSharePicture(ui)
	local thumbImagePath = self:captureThumbPicture(ui)
	return shareImagePath, thumbImagePath
end

function SharePicture:captureSharePicture(ui)
...
function SharePicture:captureThumbPicture(ui)
...
function SharePicture:__capture(path,ui)
    ...
    local yanhua1 = UIHelper:getCon(ui,"yanhua1")
    local yanhua2 = UIHelper:getCon(ui,"yanhua2")
    if yanhua1 then yanhua1:removeFromParentAndCleanup(true) end
    if yanhua2 then yanhua2:removeFromParentAndCleanup(true) end

    ui:setPosition(ccp(-25, 110))
    ui:setScale(650/630)
    ...
    parent:addChild(ui)
    ui:setScale(scale)
    ui:setPosition(ccp(savePos.x,savePos.y))
end
