QuickTableView_New = class(Layer)
local ADD_SPEED = 6000
QuickTableView_NewEventType = {
...
local innerNodeHeight = 126

function QuickTableView_New:create( width, height, renderClass)
	-- body
	local s = QuickTableView_New.new()
	s:init(width, height, renderClass)
	return s
end

function QuickTableView_New:ctor( )
	-- body
	Layer.initLayer(self)
	self.name = "QuickTableView_New"
	self.displayList = {}
...
function QuickTableView_New:init( width, height, renderClass )
	-- body
	...
	local function __getPosition( ... )
		if self.isDisposed then return ccp(0,0) end
...
function QuickTableView_New:updateData( dataList )
	-- body
...
function QuickTableView_New:updateViewByIndex( index, data )
	-- body
	if not self.displayList[index] then
		local wSize = Director:sharedDirector():getWinSize()
		local height = innerNodeHeight	-- TablevelArea.height / 5 = 106
		local width = 650
...
function QuickTableView_New:onTouchBegin( evt )
	-- body
...
function QuickTableView_New:onTouchMove( evt )
	-- body
	self.isTouching = true
	local pos = evt.globalPosition
...
function QuickTableView_New:onTouchEnd( evt )
	-- body
	local pos = evt.globalPosition
...
function QuickTableView_New:getIndexByGlobalPos( globalPosition )
	-- body
	local pos_view = self.view_content:getPosition()
...
function QuickTableView_New:onTouchTap( evt )
	-- body
	...
	local evt = Event.new(QuickTableView_NewEventType.kTapTableView, data, self)
	self:dispatchEvent(evt)
end

function QuickTableView_New:fixPos( pos )
	local tempY = pos.y
	local det_s = 0
	...
	-- 下边界
	local offset_det_max = self.isTouching and self.height/2 or (self.renderHeight/2 - 5)
...
function QuickTableView_New:checkVisible( ... )
...
function QuickTableView_New:MoveTo( pos )
	-- body
	pos = self:fixPos(pos)
...
function QuickTableView_New:initArea( index )
	-- body

	local num = math.ceil( self.height / innerNodeHeight ) 
	num = self.height / innerNodeHeight
	...
	if index > 0 and index <= #self.displayList then
		...
		self.topY = -(#self.displayList - num) * self.renderHeight + (self.renderHeight/2 )
		-- 下面不能留空
		...
		-- if _G.isLocalDevelopMode then printx(100, "initArea view_content y:",posY,"index",index,#self.displayList) end
		-- if _G.isLocalDevelopMode then printx(100, "initArea self.topY:", self.topY ) end
		-- if _G.isLocalDevelopMode then printx(100, "initArea self.bottomY:", self.bottomY ) end
...
--不是准确的 有误差 在+2>x>-2
function QuickTableView_New:getCurrentIndex( pos )
	-- body
...
-- 没有中央render，怎么播放动画。。
function QuickTableView_New:playAnimation( ... )

end

function QuickTableView_New:dispose( ... )
	-- body
	for k, v in pairs(self.speedometers) do 
		v:stopMeasure()
	end
