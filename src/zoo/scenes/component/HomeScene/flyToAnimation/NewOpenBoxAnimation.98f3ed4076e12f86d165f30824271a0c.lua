...
local GoodsFlyingAnimation = class(BasePanel)
...
local BoxOpeningAnimation = class(BasePanel)
...
function BoxOpeningAnimation:init(boxRes)
	...
	local node = ArmatureNode:create("box3")
	self.box2 = node
	...
	if boxRes then
		local slot = self.box2:getCon('bg')
		boxRes:setAnchorPoint(ccp(0.5, 0.5))
		-- slot:setDisplayImage(boxRes)
		slot:addChild(boxRes)
		boxRes:setPosition(ccp(202/2, 181/2))
	end
end
...
NewOpenBoxAnimation = class(BasePanel)

function NewOpenBoxAnimation:create(rewards, boxRes, specialItemsConfig, showNum)
	local instance = NewOpenBoxAnimation.new()
	instance:loadRequiredResource("ui/openbox.json")
	instance:init(rewards, boxRes, specialItemsConfig, showNum)
	return instance
end

function NewOpenBoxAnimation:init(rewards, boxRes, specialItemsConfig, showNum)
	local ui = Layer:create()
	self.ui = ui
	BasePanel.init(self, ui)
...
function NewOpenBoxAnimation:close()
	PopoutManager:remove(self, true)
end

function NewOpenBoxAnimation:play()
...
function NewOpenBoxAnimation:setFinishCallback( finishCallback )
